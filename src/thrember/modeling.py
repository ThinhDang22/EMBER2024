...
# Params mà PassiveAggressiveClassifier chấp nhận — filter trước khi truyền vào.
_PA_VALID_PARAMS = {"C", "fit_intercept", "tol", "shuffle", "random_state", "average", "n_jobs", "max_iter"}
...
def _best_binary_threshold_by_f1(y_true, scores):
    ...
    if len(y_true) == 0 or len(scores) == 0:
        return 0.5, 0.0
    candidate_thresholds = np.unique(np.quantile(scores, np.linspace(0.01, 0.99, 99)))
...
class UnifiedModel:
    ...
    def _build_sklearn_model(self):
        if self.model_type == "logreg":
            return make_pipeline(StandardScaler(with_mean=False), LogisticRegression(**self.params))

        # FIX: 3 SGD variants đều giống nhau — collapse thành 1 block.
        if self.model_type in {"sgd_logreg", "sgd_huber", "sgd_hinge"}:
            return make_pipeline(StandardScaler(with_mean=False), SGDClassifier(**self.params))

        if self.model_type == "ridge":
            return make_pipeline(StandardScaler(with_mean=False), RidgeClassifier(**self.params))

        if self.model_type == "rf":
            return RandomForestClassifier(**self.params)

        if self.model_type == "mlp":
            return make_pipeline(StandardScaler(with_mean=False), MLPClassifier(**self.params))
        ...
        # CatBoost: tree-based, không cần StandardScaler. Import lazy — optional dep.
    ...
    def fit(self, x_train, y_train, x_val=None, y_val=None, sample_weight=None, val_sample_weight=None) -> "UnifiedModel":
        if self.model_type == "lgbm":
            ...
            # CatBoost cần eval_set để early_stopping_rounds hoạt động đúng.
            # x_val/y_val đã được split sẵn trong train_classifier (validation_size=0.1).
        ...
        if self.problem_type == "binary" and x_val is not None and y_val is not None and len(y_val) > 0:
            try:
                ...
                _status(f"validation threshold selected: threshold={self.threshold:.6f}, f1={self.validation_f1:.6f}")
            except Exception as exc:
                _status(f"could not select validation threshold: {exc}")

        return self

    def _fit_lightgbm(self, x_train, y_train, x_val=None, y_val=None, sample_weight=None, val_sample_weight=None):
        ...
        if self.problem_type == "binary":
            ...
            # FIX: dùng len(unique) thay vì max+1 — tránh bug khi có class gap
            # sau khi _maybe_sample() cắt data và một số class biến mất.
            params["num_class"] = int(len(np.unique(y_train)))
        else:
            raise ValueError(f"LightGBM supports binary/multiclass only, got {self.problem_type}")

        train_set = lgb.Dataset(x_train, y_train, weight=sample_weight,
                                categorical_feature=self.categorical_features, free_raw_data=True)

        valid_sets, valid_names, callbacks = None, None, []
        if x_val is not None and y_val is not None and len(y_val) > 0:
            val_set = lgb.Dataset(x_val, y_val, weight=val_sample_weight, reference=train_set,
                                  categorical_feature=self.categorical_features, free_raw_data=True)
            valid_sets = [val_set]
            valid_names = ["valid"]
        ...
        return lgb.train(params, train_set, valid_sets=valid_sets, valid_names=valid_names, callbacks=callbacks or None)
    ...
    def predict_scores(self, x):
        ...
        if isinstance(self.model, dict) and self.model.get("kind") == "sgd_stream":
            ...
            if hasattr(classifier, "predict_proba"):
                ...
                return probs[:, 1] if self.problem_type == "binary" else probs
    ...
    def save(self, path: Path | str) -> None:
        ...
        if self.model_type == "lgbm":
            ...
            with meta_path.open("w", encoding="utf-8") as fout:
                json.dump({"model_type": self.model_type, "problem_type": self.problem_type,
                           "params": self.params, "threshold": self.threshold,
                           "validation_f1": self.validation_f1}, fout, indent=2, ensure_ascii=False)
            return
        joblib.dump({"model_type": self.model_type, "problem_type": self.problem_type,
                     "params": self.params, "model": self.model,
                     "threshold": self.threshold, "validation_f1": self.validation_f1}, path)
    ...
    def load(cls, path: Path | str, model_type: str | None = None, problem_type: str | None = None) -> "UnifiedModel":
        ...
        wrapper = cls(model_type=payload["model_type"], problem_type=payload["problem_type"],
...
def _maybe_sample(x, y, sample_size: int | None, random_state: int = 42):
    ...
    if y.ndim == 1 and len(np.unique(y)) > 1:
        selected, _ = train_test_split(idx, train_size=sample_size, random_state=random_state, stratify=y)
    else:
        rng = np.random.default_rng(random_state)
        selected = rng.choice(idx, size=sample_size, replace=False)
    return x[np.sort(selected)], y[np.sort(selected)]


def _make_sample_weight(y, mode: str | None = None, max_weight: float = 20.0):
    """Create per-sample class weights.
    Modes: None/none = off | balanced | balanced_sqrt (safer for extreme imbalance)
    """
    ...
    n_samples, n_classes = float(len(y)), float(len(classes))
    weight_by_class = {int(cls): n_samples / (n_classes * float(cnt)) for cls, cnt in zip(classes, counts)}
    sample_weight = np.asarray([weight_by_class[int(label)] for label in y], dtype=np.float32)
    if mode == "balanced_sqrt":
        sample_weight = np.sqrt(sample_weight)
    elif mode != "balanced":
        raise ValueError(f"Unsupported class_weight_mode={mode}. Use: none, balanced, balanced_sqrt")
...
def _make_class_weight_lookup(y, mode: str | None = None, max_weight: float = 20.0):
    ...
    if weights is None:
        return None
    y = np.asarray(y)
    return {int(label): float(weight) for label, weight in zip(y, weights)}
...
def _train_streaming_sgd_classifier(
    x_raw, y_raw, model_type: str, params: dict[str, Any] | None,
    problem_type: str, random_state: int, sample_size: int | None = None,
) -> UnifiedModel:
    """Train PA hoặc SGDClassifier incrementally trên memmap-backed data.
    Chỉ một chunk X được load vào RAM tại một thời điểm — tránh OOM.
    """
    if problem_type != "multiclass":
        raise ValueError("streaming path is currently intended for Layer 2 multiclass training only.")
    ...
    if sample_size is not None and sample_size < len(keep_idx):
        ...
        if len(np.unique(y_keep)) > 1:
            keep_idx, _ = train_test_split(keep_idx, train_size=sample_size,
                                           random_state=random_state, stratify=y_keep)
        else:
            rng = np.random.default_rng(random_state)
    ...
    weight_lookup = _make_class_weight_lookup(y_train_all, mode=class_weight_mode, max_weight=class_weight_max)

    # Chọn classifier theo model_type.
    if model_type == "pa":
        pa_params = {k: v for k, v in fit_params.items() if k in _PA_VALID_PARAMS}
        classifier = PassiveAggressiveClassifier(**pa_params)
        algo_label = "PassiveAggressive"
    else:
        classifier = SGDClassifier(**fit_params)
        algo_label = "SGD"

    _status(
        f"streaming {algo_label} | samples={len(keep_idx)} | classes={len(classes)} | "
        f"epochs={epochs} | chunk_size={chunk_size} | weight_mode={class_weight_mode}"
    )

    scaler = StandardScaler(with_mean=False, copy=False)
    sorted_idx = np.sort(keep_idx)
    _status("streaming scaler pass")
    ...
    for epoch in range(1, max(1, epochs) + 1):
        ...
        _status(f"streaming {algo_label} epoch {epoch}/{epochs}")
    ...
    wrapper.model = {"kind": "sgd_stream", "scaler": scaler, "classifier": classifier, "classes": classes}
    return wrapper
...
def train_classifier(
    ...
    if y.ndim != 1:
        raise ValueError("train_classifier expects 1D labels.")
    if len(y) == 0:
    ...
    if problem_type is None:
        ...
        if len(labeled_y) == 0:
            raise ValueError("No labeled rows found in training data.")
        problem_type = "binary" if int(len(np.unique(labeled_y))) <= 2 else "multiclass"

    # Streaming path: PA và SGD variants — tránh OOM của sklearn full-fit.
    if model_type.lower() in {"sgd_logreg", "sgd_huber", "sgd_hinge", "pa"} and problem_type == "multiclass":
        return _train_streaming_sgd_classifier(
            x_raw=x, y_raw=y, model_type=model_type, params=params,
            problem_type=problem_type, random_state=random_state, sample_size=sample_size,
        )
    ...
    sample_weight = _make_sample_weight(y_train, mode=class_weight_mode, max_weight=class_weight_max)
    val_sample_weight = None
    if y_val is not None:
        val_sample_weight = _make_sample_weight(y_val, mode=class_weight_mode, max_weight=class_weight_max)

    if sample_weight is not None:
        _status(f"sample weight enabled | mode={class_weight_mode} | "
                f"min={float(np.min(sample_weight)):.4f} | max={float(np.max(sample_weight)):.4f} | "
    ...
    model.fit(x_train, y_train, x_val, y_val, sample_weight=sample_weight, val_sample_weight=val_sample_weight)
    _status("fit done")
    return model


def train_multilabel_ovr(
    data_dir: Path | str, base_model_type: str = "lgbm", params: dict[str, Any] | None = None,
    validation_size: float = 0.1, random_state: int = 42, sample_size: int | None = None,
) -> list[UnifiedModel]:
    _status(f"load train data: {data_dir}")
    x, y = read_vectorized_features(data_dir, "train")
    x, y = _filter_labeled_rows(x, y)
    ...
    for col in range(y.shape[1]):
        ...
        model = UnifiedModel(model_type=base_model_type, problem_type="binary", params=params)
...
def evaluate_classifier(
    model: UnifiedModel, data_dir: Path | str, subset: str = "test", threshold: float | None = None,
) -> dict[str, Any]:
    ...
    if model.problem_type == "binary":
        ...
        used_threshold = threshold if threshold is not None else getattr(model, "threshold", 0.5)
        ...
        return {
            ...
            "tpr_at_fpr_1pct": float(_tpr_at_max_fpr(fpr, tpr, max_fpr=0.01)),
            ...
            "validation_f1": None if getattr(model, "validation_f1", None) is None else float(model.validation_f1),
    ...
    metrics = {
        ...
        "num_true_classes_in_eval": int(len(np.unique(y))),
        "num_predicted_classes_in_eval": int(len(np.unique(labels))),
...
def evaluate_multilabel_ovr(
    models: list[UnifiedModel], data_dir: Path | str, subset: str = "test", threshold: float = 0.5,
) -> dict[str, float]:
    _status(f"load eval data: {data_dir} [{subset}]")
    ...
    pred_cols = [(np.asarray(model.predict_scores(x)) >= threshold).astype(np.int32) for model in models]
...
def tune_classifier(
    data_dir: Path | str, model_type: str = "lgbm", problem_type: str = "binary",
    sample_size: int | None = 200000, random_state: int = 42,
) -> dict[str, Any]:
    if model_type != "lgbm":
    ...
    param_grid = {
        ...
        "num_leaves": [64, 128, 256], "feature_fraction": [0.8, 1.0], "bagging_fraction": [0.8, 1.0],
    }
    grid = GridSearchCV(estimator=estimator, cv=progressive_cv, param_grid=param_grid,
                        scoring=make_binary_auc_scorer(), n_jobs=1, verbose=2)
